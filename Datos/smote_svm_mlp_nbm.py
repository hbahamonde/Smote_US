...
RepeatSVMRBF = np.zeros([nrepeat,nfold])
RepeatSVMSigmoid = np.zeros([nrepeat,nfold])
RepeatGNB = np.zeros([nrepeat,nfold])
RepeatMLP = np.zeros([nrepeat,nfold])

PPVSVMLinear = np.zeros([nrepeat,nfold])
PPVSVMRBF = np.zeros([nrepeat,nfold])
PPVSVMSigmoid = np.zeros([nrepeat,nfold])
PPVGNB = np.zeros([nrepeat,nfold])
PPVMLP = np.zeros([nrepeat,nfold])
...
for i in range(nrepeat):
    ...
    for j in range(nfold):
        ...
        linear_svc = svm.SVC(kernel='linear',random_state=0, tol=1e-5, C = 1)
        ...
        rbf_svc = svm.SVC(kernel='rbf',random_state=0, tol=1e-5, C = 1)
        rbf_svc.fit(X_train, y_train)

        y_predict = rbf_svc.predict(X_test)
        tn, fp, fn, tp = confusion_matrix(y_test.ravel(),y_predict).ravel()
        RepeatSVMRBF[i,j] = tp/(tp+fn)
        PPVSVMRBF[i,j] = tp/(tp+fp)

        sigmoid_svc = svm.SVC(kernel='sigmoid',random_state=0, tol=1e-5, C = 1)
        sigmoid_svc.fit(X_train, y_train)

        y_predict = sigmoid_svc.predict(X_test)
        tn, fp, fn, tp = confusion_matrix(y_test.ravel(),y_predict).ravel()
        RepeatSVMSigmoid[i,j] = tp/(tp+fn)
        PPVSVMSigmoid[i,j] = tp/(tp+fp)

        mlp = MLPClassifier(solver='sgd',
                            max_iter=1000,alpha=1e-5,hidden_layer_sizes=(50,50,50,), random_state=1)
        mlp.fit(X_train, y_train)
        y_predict = mlp.predict(X_test)
        tn, fp, fn, tp = confusion_matrix(y_test.ravel(),y_predict).ravel()
        RepeatMLP[i,j] = tp/(tp+fn)
        PPVMLP[i,j] = tp/(tp+fp)
...
print('RBF')
print(mean(100*RepeatSVMRBF.ravel()))
print(pstdev(100*RepeatSVMRBF.ravel()))
print(mean(100*PPVSVMRBF.ravel()))
print(pstdev(100*PPVSVMRBF.ravel()))

print('Sigmoid')
print(mean(100*RepeatSVMSigmoid.ravel()))
print(pstdev(100*RepeatSVMSigmoid.ravel()))
print(mean(100*PPVSVMSigmoid.ravel()))
print(pstdev(100*PPVSVMSigmoid.ravel()))

print('MLP classifier')
print(mean(100*RepeatMLP.ravel()))
print(pstdev(100*RepeatMLP.ravel()))
print(mean(100*PPVMLP.ravel()))
print(pstdev(100*PPVMLP.ravel()))

print('Naive Bayes')
print(mean(100*RepeatGNB.ravel()))
print(pstdev(100*RepeatGNB.ravel()))
print(mean(100*PPVGNB.ravel()))
print(pstdev(100*PPVGNB.ravel()))
